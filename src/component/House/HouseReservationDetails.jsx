...
const HouseReservationDetails = () => {
  const {  imgBaseUrl,baseUrl, token } = useContext(MainContext);
  const{houseReservations} = useParams();
  ...
  const fetchUserProperties =  async () =>
    {
      try {
        const response = await Axios({
          method: 'get',
          url: `${baseUrl}vendor-hotel/admin-reservation?reservationId=${houseReservations}`,
          headers: 
          {
          ...
          <h3 style={{marginBottom: 20}}>Property details</h3>
              ...
              <div style={{position: 'relative'}}>
                ...
                <Form layout='inline' className='flex flex-align driver-details-form-header' >
                      ...
                      <div style={{position: 'absolute', top: 0, right: 10, fontSize: 12, fontWeight: 600, color: '#666'}}><Link to={`/dashboard/hotel/notice/${property?.reservation?._id}`}>View Notices</Link></div>
              </div>
...
export default HouseReservationDetails