...
const { Option } = Select;
const HotelPropertiesDetails = () => {
  const {  imgBaseUrl,baseUrl, token } = useContext(MainContext);
  const{hotelProperties} = useParams();
  ...
  const fetchUserProperties =  async () =>
    {
      try {
        const response = await Axios({
          method: 'get',
          url: `${baseUrl}vendor-hotel/admin-hospitality-by-user?userId=${hotelProperties}`,
          headers: 
          {
...
export default HotelPropertiesDetails